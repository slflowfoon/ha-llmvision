...
class ServiceCallData:
    ...
    def __init__(self, data_call):
        ...
        self.expose_video = data_call.data.get(EXPOSE_VIDEO, False)
        self.video_target_fps = int(data_call.data.get(VIDEO_TARGET_FPS, 4))
        self.video_target_width = int(data_call.data.get(VIDEO_TARGET_WIDTH, 512))
...
def setup(hass, config):
    ...
    async def stream_analyzer(data_call):
        ...
        client = await processor.add_streams(image_entities=call.image_entities,
                                             ...
                                             include_filename=call.include_filename,
                                             expose_images=call.expose_images,
                                             expose_video=call.expose_video,
                                             video_target_fps=call.video_target_fps,
                                             video_target_width=call.video_target_width
                                             )
